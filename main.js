...
var config = JSON.parse(
  fs.readFileSync(
    path.join(process.cwd(), process.argv[2], "config.json"),
  ),
);
var componentList = config.components;
var stylesheets = []
var basePath = path.join(process.cwd(), process.argv[2]);
function uniq(a) {
  var seen = {};
...
function generateComponent(template, script, properties, name) {
  return new Promise((res) => {
    ...
    if (componentList[name].stylesheets && componentList[name].page && !(stylesheets.includes(componentList[name].stylesheets))) { 
      stylesheets.push(componentList[name].stylesheets)
      style=`var head=document.getElementByTagName('head')[0];var styles=${JSON.stringify(componentList[name].stylesheets)};for(var i=0;i<styles.length;i++){var link=document.createElement('link');link.href='./'+styles[i]+'.css';head.appendChild(link)};`
     }
      ...
      Array.from(tempDom.getElementsByTagName(tag)).forEach(async (node) => {
        ...
        if (includes.length - 1 == i) {
          ...
          func = new Function(
            ...
            `${style}var props=arguments[0];var funcs={${tags.slice(0, -1)}};var container=document.createElement('div');${basicProps}var component;${script}${props}component=\`${template}\`.replaceAll('$[children]',props.children);container.innerHTML+=component;Array.from(container.getElementsByTagName('comp')).forEach((elm)=>{elm.replaceWith(new Function('func',\`return func.\${elm.className}(\${elm.getAttribute('params')})\`)(funcs))});${script2}${pageInsert}return container`,
          );
    ...
    if (includes.length == 0) {
      func = new Function(
        ...
        `${style}var props=arguments[0];var container=document.createElement('div');${basicProps}var component;${script}${props}component=\`${template}\`.replaceAll('$[children]',props.children);container.innerHTML+=component;${script2}${pageInsert}return container`,
      );
      res(genNamedFunc(name, func));
    }
    //console.log()
